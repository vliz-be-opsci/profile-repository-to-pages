    ...
    try:
        ...
        if response.status_code == 200:
            if "application/json" in response.headers["Content-Type"]:
                ...
                logger.error(
                    f"URI returned {response.headers['Content-Type']} instead of application/json"
                )
                return False
        else:
...
def check_uri_content(uri):
    ...
    if it is not valid it will send out a request with the response header
    set to application/json and check if the response is valid
    if not send out a normal request and check if the response is valid
    if valid then check in the head of the html of there is a
    <link href="./ro-crate-metadata.json" rel="describedby" type="application/ld+json"> tag
    ...
    try:
        ...
        if response.status_code == 200:
            # check if the html contains a <link href="./ro-crate-metadata.json"
            # rel="describedby" type="application/ld+json"> tag
            # ./ro-crate-metadata.json can be anything so regex will be used
            #   even htts://www.google.com/ro-crate-metadata.json
            regex = r"<link href=\"(.*)\" rel=\"describedby\" type=\"application/ld\+json\">"
            matches = re.finditer(regex, response.text, re.MULTILINE)
            for matchNum, match in enumerate(matches, start=1):
                logger.info(
                    f'URI {uri} contains a <link href="{match.group(1)}" rel="describedby" type="application/ld+json"> tag'
                )
                return match.group(1)
            logger.error(
                f'URI {uri} does not contain a <link href="./ro-crate-metadata.json" rel="describedby" type="application/ld+json"> tag'
            )
            return False
        else:
            logger.error(f"URI {uri} is not valid")
